  ...
  setsData.forEach((setData, idx) => {
    ...
    checkBtn.addEventListener('click', () => {
      ...
      if (setData.completed) {
        playLoudBeep(880, 'square', 0.15, 0.9);
        triggerVibration([80]);

        // Si faltan series dentro del mismo ejercicio
        if (idx < setsData.length - 1) {
          launchRestTimer(ex.restSeconds, `Próxima: Serie ${idx + 2} de ${ex.title}`);
        } 
        // Si es la última serie del ejercicio y hay un siguiente ejercicio
        else if (state.currentExerciseIdx < ROUTINE.length - 1) {
          const currentRest = ex.restSeconds;
          state.currentExerciseIdx++;
          renderCurrentExerciseView();
          const nextEx = ROUTINE[state.currentExerciseIdx];
          launchRestTimer(currentRest, `Descanso entre ejercicios ➔ Próximo: ${nextEx.title}`);
        }
      }
    });
...
function navigateNextExercise() {
  if (state.currentExerciseIdx < ROUTINE.length - 1) {
    const currentEx = ROUTINE[state.currentExerciseIdx];
    state.currentExerciseIdx++;
    renderCurrentExerciseView();
    const nextEx = ROUTINE[state.currentExerciseIdx];
    launchRestTimer(currentEx.restSeconds, `Descanso entre ejercicios ➔ Próximo: ${nextEx.title}`);
  } else {
